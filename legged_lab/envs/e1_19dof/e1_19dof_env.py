...
# All rights reserved.
#
# Copyright (c) 2025-2026, The E1_19DOF-Lab Project Developers.
# All rights reserved.
# Modifications are licensed under the BSD-3-Clause license.
#
# This file contains code derived from the RSL-RL, Isaac Lab, and Legged Lab Projects,
# with additional modifications by the E1_19DOF-Lab Project,
...
# 🔴 修改为 19dof 路径
from legged_lab.envs.e1_19dof.walk_cfg import E1_19DOF_WalkFlatEnvCfg
...
class E1_19DOF_Env(VecEnv):
    ...
    def init_buffers(self):
        ...
        # 🔴 移除了 elbow_link，用 shoulder_yaw_link 作为手臂的假想末端，用来估算 hand_pos
        self.hand_body_ids, _ = self.robot.find_bodies(
            name_keys=["left_shoulder_yaw_link", "right_shoulder_yaw_link"], preserve_order=True
        )
        ...
        # 🔴 移除了 left_elbow_joint 和 right_elbow_joint
    ...
    def visualize_motion(self, time):
        ...
        """
        # 1. 抓取当前时刻的完整动捕帧。19 DOF 数据，每一帧的向量长度应该是 50。
        ...
        # 🔴 手臂现在各占 3 DOF，切片范围相应收缩
        dof_pos[:, self.left_arm_ids] = visual_motion_frame[19:22]     # 19,20,21 (3 DOF)
        dof_pos[:, self.right_arm_ids] = visual_motion_frame[22:25]    # 22,23,24 (3 DOF)

        # ====== 👇 3. 解析关节速度 (DOF VEL) - 19DOF 重新切片 👇 ======
        # 位置数据结束于 24。紧接着：
        # 索引 25-27: Root Lin Vel
        # 索引 28-30: Root Ang Vel 
        
        dof_vel[:, self.left_leg_ids] = visual_motion_frame[31:37]     # 31,32,33,34,35,36 (6 DOF)
        dof_vel[:, self.right_leg_ids] = visual_motion_frame[37:43]    # 37,38,39,40,41,42 (6 DOF)
        dof_vel[:, self.waist_ids] = visual_motion_frame[43:44]        # 43 (1 DOF)
        dof_vel[:, self.left_arm_ids] = visual_motion_frame[44:47]     # 44,45,46 (3 DOF)
        dof_vel[:, self.right_arm_ids] = visual_motion_frame[47:50]    # 47,48,49 (3 DOF)
        ...
        lin_vel = visual_motion_frame[25:28].clone()  # 🔴 偏移量更新
        ang_vel = visual_motion_frame[28:31].clone()  # 🔴 偏移量更新
        ...
        # 使用新的 hand_body_ids (也就是 shoulder_yaw_link) 计算位置
    ...
    def compute_current_observations(self):
        ...
        current_actor_obs = torch.cat(
            [
                ...
                joint_pos * self.obs_scales.joint_pos,  # 19
                joint_vel * self.obs_scales.joint_vel,  # 19
                action * self.obs_scales.actions,  # 19
