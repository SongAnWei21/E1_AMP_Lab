...
# All rights reserved.
#
# Copyright (c) 2025-2026, The E1_21DOF-Lab Project Developers.
# All rights reserved.
# Modifications are licensed under the BSD-3-Clause license.
#
# This file contains code derived from the RSL-RL, Isaac Lab, and Legged Lab Projects,
# with additional modifications by the E1_21DOF-Lab Project,
...
from legged_lab.envs.e1_21dof.run_cfg import E1_21DOF_RunFlatEnvCfg
from legged_lab.envs.e1_21dof.walk_cfg import E1_21DOF_WalkFlatEnvCfg
...
class E1_21DOF_Env(VecEnv):
    def __init__(
        self,
        cfg: (
            E1_21DOF_RunFlatEnvCfg
    ...
    def init_buffers(self):
        ...
        self.torso_ids, _ = self.robot.find_joints(
            name_keys=["waist_yaw_joint"], # 替换为你实际的腰部关节名
            preserve_order=True,
        )
        ...
        # 手臂是沿着正 X 轴生长的,长度取 0.25 米作为手腕估计点
        self.left_arm_local_vec = torch.tensor([0.25, 0.0, 0.0], device=self.device).repeat((self.num_envs, 1))
        self.right_arm_local_vec = torch.tensor([0.25, 0.0, 0.0], device=self.device).repeat((self.num_envs, 1))
    ...
    def visualize_motion(self, time):
        ...
        Assumes the input motion data (small_forward_boy.txt) is generated from a 
        True 21 DOF data capture, exactly matching the E1_v2 robot topology.
        """
        # 1. 抓取当前时刻的完整动捕帧。 21 DOF 数据，每一帧的向量长度应该是 54。
        visual_motion_frame = self.amp_loader_display.get_full_frame_at_time(0, time)
        device = self.device

        # 如果这里报错或动作依然鬼畜，请注释掉第 4 步和第 5 步，在此处执行：
        # print(f"[DEBUG] Loaded frame shape: {visual_motion_frame.shape}")
        # 并确认其长度是否为 54 (3+3+21 + 3+3+21)。
        ...
        # ====== 👇 2. 解析关节位置 (DOF POS) - 全维精准切片 👇 ======
        # 索引 0-5: Root Pos, Root Euler (跳过关节处理部分)
        
        dof_pos[:, self.left_leg_ids] = visual_motion_frame[6:12]      # 6,7,8,9,10,11 (6 DOF)
        dof_pos[:, self.right_leg_ids] = visual_motion_frame[12:18]    # 12,13,14,15,16,17 (6 DOF)
        
        # [核心修正] 动捕文件中真正的第 18 位数据归属于腰部！
        dof_pos[:, self.waist_ids] = visual_motion_frame[18:19]       # 18 (1 DOF)
        
        dof_pos[:, self.left_arm_ids] = visual_motion_frame[19:23]    # 19,20,21,22 (4 DOF)
        dof_pos[:, self.right_arm_ids] = visual_motion_frame[23:27]   # 23,24,25,26 (4 DOF)

        # ====== 👇 3. 解析关节速度 (DOF VEL) - 全维精准切片 👇 ======
        # 位置数据结束于 26。紧接着：
        # 索引 27-29: Root Lin Vel
        # 索引 30-32: Root Ang Vel (Ang Vel 通常默认全 0)
        
        dof_vel[:, self.left_leg_ids] = visual_motion_frame[33:39]    # 33,34,35,36,37,38
        dof_vel[:, self.right_leg_ids] = visual_motion_frame[39:45]   # 39,40,41,42,43,44
        
        # [核心修正] 动捕文件中真正的第 45 位速度数据归属于腰部！
        dof_vel[:, self.waist_ids] = visual_motion_frame[45:46]       # 45

        dof_vel[:, self.left_arm_ids] = visual_motion_frame[46:50]    # 46,47,48,49
        dof_vel[:, self.right_arm_ids] = visual_motion_frame[50:54]   # 50,51,52,53 (真正的最后一节速度)

        self.robot.write_joint_position_to_sim(dof_pos)
        self.robot.write_joint_velocity_to_sim(dof_vel)

        # ====== 4. 解析根节点状态 (Root State) ======
        # 位置数据 (3) + 欧拉角 (3) = 6，索引结束于 5。
        # 关节位置数据 (21) = 索引 6-26。
        # 根节点速度数据开始于 27。
        ...
        ang_vel = visual_motion_frame[30:33].clone() # 通常角速度也是动捕文件里录好的，可以直接用
        ...
        self.robot.write_root_state_to_sim(root_state, torch.arange(self.num_envs, device=device))
        self.sim.render()
        self.sim.step()
        self.scene.update(dt=self.step_dt)

        # ====== 5. 计算并拼合 21 DOF 的 AMP 专家观测数据供返回 ======
        # 此处使用实时仿真器数据，必须是完整的 21 自由度！
        left_hand_pos = quat_apply(
            quat_conjugate(self.robot.data.root_state_w[:, 3:7]),
            self.robot.data.body_state_w[:, self.elbow_body_ids[0], :3] - self.robot.data.root_state_w[:, 0:3] + 
            quat_apply(self.robot.data.body_state_w[:, self.elbow_body_ids[0], 3:7], self.left_arm_local_vec)
        )
        right_hand_pos = quat_apply(
            quat_conjugate(self.robot.data.root_state_w[:, 3:7]),
            self.robot.data.body_state_w[:, self.elbow_body_ids[1], :3] - self.robot.data.root_state_w[:, 0:3] + 
        ...
        # return torch.cat(
        #     (
        #         self.robot.data.joint_pos[:, self.right_arm_ids],
        #         self.robot.data.joint_pos[:, self.left_arm_ids],
        #         self.robot.data.joint_pos[:, self.waist_ids],        # 实时的腰部数据
        #         self.robot.data.joint_pos[:, self.right_leg_ids],
        #         self.robot.data.joint_pos[:, self.left_leg_ids],
        #         self.robot.data.joint_vel[:, self.right_arm_ids],
        #         self.robot.data.joint_vel[:, self.left_arm_ids],
        #         self.robot.data.joint_vel[:, self.waist_ids],        # 实时的腰部速度数据
        #         self.robot.data.joint_vel[:, self.right_leg_ids],
        #         self.robot.data.joint_vel[:, self.left_leg_ids],
        #         left_hand_pos,
        #         right_hand_pos,
        #         left_foot_pos,
        #         right_foot_pos
        ...
        return torch.cat(
            (
                ...
                self.robot.data.joint_pos[:, self.left_leg_ids],
                self.robot.data.joint_pos[:, self.right_leg_ids],
                self.robot.data.joint_pos[:, self.waist_ids],
                self.robot.data.joint_pos[:, self.left_arm_ids],
                self.robot.data.joint_pos[:, self.right_arm_ids],
                ...
                self.robot.data.joint_vel[:, self.left_leg_ids],
                self.robot.data.joint_vel[:, self.right_leg_ids],
                self.robot.data.joint_vel[:, self.waist_ids],
                self.robot.data.joint_vel[:, self.left_arm_ids],
                self.robot.data.joint_vel[:, self.right_arm_ids],
