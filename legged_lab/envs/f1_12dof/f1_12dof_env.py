...
# with additional modifications by the F1_12DOF-Lab Project,
...
# 更新了导入路径为 f1_12dof
from legged_lab.envs.f1_12dof.walk_cfg import F1_12DOF_WalkFlatEnvCfg
...
class F1_12DOF_Env(VecEnv):
    ...
    def init_buffers(self):
        ...
        # 严格匹配 F1 URDF 中的脚底连杆名称 (注意大写的 Link)
        self.feet_body_ids, _ = self.robot.find_bodies(
            name_keys=["l_ankle_roll_Link", "r_ankle_roll_Link"], preserve_order=True
        )

        # 严格匹配 F1 URDF 中的腿部关节名称与顺序 (包含 knee_yaw)
        ...
        self.ankle_joint_ids, _ = self.robot.find_joints(
            name_keys=["l_ankle_pitch_joint", "r_ankle_pitch_joint", "l_ankle_roll_joint", "r_ankle_roll_joint"],
            preserve_order=True,
        )
    ...
    def visualize_motion(self, time):
        ...
        # ====== 12 DOF 数据切片 ======
        # TXT 包含：Root(0:6), LegPos(6:18), RootVel(18:24), LegVel(24:36)
        ...
        ang_vel = visual_motion_frame[21:24].clone()
        ...
        # 严格按照 [左腿12, 右腿12, 左腿速度12, 右腿速度12, 左脚3, 右脚3] 的顺序拼接返回 (共 30 维)
        return torch.cat(
            (
