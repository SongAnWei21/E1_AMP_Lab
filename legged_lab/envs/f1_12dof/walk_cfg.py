...
# All rights reserved.
#
# Copyright (c) 2025-2026, The F1_12DOF-Lab Project Developers.
# All rights reserved.
...
# 🔴 请确保你有一个 f1_12dof 的资产配置文件，并正确导入 F1_12DOF_CFG
from legged_lab.assets.f1_12dof import F1_12DOF_CFG 
...
class F1_12DOFRewardCfg:
    ...
    undesired_contacts = RewTerm(
        ...
        params={
            ...
            "sensor_cfg": SceneEntityCfg(
                "contact_sensor", body_names=[".*knee.*", "base_link"]
            ),
            "threshold": 1.0,
        },
    )
    ...
    # 🔴 F1 没有 hip_yaw，但有 knee_yaw，因此惩罚这两个产生横向扭转的关节
    joint_deviation_hip_knee_yaw = RewTerm(
...
@configclass
class F1_12DOF_WalkFlatEnvCfg:
    # 路径修改为 f1_12dof
    amp_motion_files_display = ["legged_lab/envs/f1_12dof/datasets/motion_visualization/walk_1300fps.txt"]
    device: str = "cuda:0"
    scene: BaseSceneCfg = BaseSceneCfg(
        max_episode_length_s=20.0,
        num_envs=4096,
        env_spacing=2.5,
        robot=F1_12DOF_CFG,  # 使用修改后的 F1 配置
        ...
        height_scanner=HeightScannerCfg(
            ...
            prim_body_name="base_link",  # 🔴 改为 F1 的 base_link
            resolution=0.1,
            size=(1.6, 1.0),
            debug_vis=False,
    ...
    robot: RobotCfg = RobotCfg(
        ...
        action_scale=0.25,
        terminate_contacts_body_names=[".*knee.*", "base_link"], # 🔴 匹配 F1 躯干
        feet_body_names=[".*ankle_roll_Link"], # 🔴 匹配 F1 脚底
    )
    reward = F1_12DOFRewardCfg()
    gait = GaitCfg()
    ...
    domain_rand: DomainRandCfg = DomainRandCfg(
        events=EventCfg(
            ...
            add_base_mass=EventTerm(
                ...
                mode="startup",
                params={
                    "asset_cfg": SceneEntityCfg("robot", body_names="base_link"), # 🔴 匹配 F1 躯干
...
@configclass
class F1_12DOF_WalkAgentCfg(RslRlOnPolicyRunnerCfg):
    seed = 42
    device = "cuda:0"
    ...
    experiment_name = "f1_12dof_walk"  # 🔴 修改为 F1 实验名称
    run_name = ""
    logger = "tensorboard"
    neptune_project = "f1_12dof_walk"
    wandb_project = "f1_12dof_walk"
    resume = False
    load_run = ".*"
    ...
    amp_reward_coef = 0.3
    # 🔴 路径修改为 f1_12dof
    amp_motion_files = ["legged_lab/envs/f1_12dof/datasets/motion_amp_expert/walk_1300fps.txt"]
