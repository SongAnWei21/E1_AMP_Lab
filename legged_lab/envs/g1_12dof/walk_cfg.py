...
# All rights reserved.
#
# Copyright (c) 2025-2026, The G1_12DOF-Lab Project Developers.
# All rights reserved.
...
# 导入修改好的 G1 资产配置文件
from legged_lab.assets.g1_12dof import G1_12DOF_CFG
...
class GaitCfg:
    ...
    gait_cycle: float = 0.85 # 恢复标准步频，防止高频倒退


@configclass
class G1_12DOFRewardCfg:
    # --- 任务目标 ---
    ...
    undesired_contacts = RewTerm(
        ...
        params={
            "sensor_cfg": SceneEntityCfg(
                "contact_sensor", body_names=[".*knee.*", ".*pelvis.*"]
            ),
            "threshold": 1.0,
        },
    )
...
@configclass
class G1_12DOFWalkFlatEnvCfg:

    amp_motion_files_display = ["legged_lab/envs/g1_12dof/datasets/motion_visualization/small_forward_boy.txt"]
    device: str = "cuda:0"
    scene: BaseSceneCfg = BaseSceneCfg(
        max_episode_length_s=20.0,
        num_envs=4096,
        env_spacing=2.5,
        robot=G1_12DOF_CFG,  # 使用 G1 的模型配置
    ...
    robot: RobotCfg = RobotCfg(
        ...
        action_scale=0.25,
        terminate_contacts_body_names=[".*knee.*", ".*pelvis.*"],
        feet_body_names=[".*ankle_roll.*"],
    )
    reward = G1_12DOFRewardCfg()
    gait = GaitCfg()
...
@configclass
class G1_12DOFWalkAgentCfg(RslRlOnPolicyRunnerCfg):
    seed = 42
    device = "cuda:0"
    ...
    experiment_name = "g1_12dof_walk"  # 修改了实验名称
    run_name = ""
    logger = "tensorboard"
    neptune_project = "g1_12dof_walk"
    wandb_project = "g1_12dof_walk"
    resume = False
    load_run = ".*"
    ...
    amp_motion_files = ["legged_lab/envs/g1_12dof/datasets/motion_amp_expert/small_forward_boy.txt"]
