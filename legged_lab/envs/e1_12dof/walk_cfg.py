...
from legged_lab.assets.e1_12dof import E1_12DOF_CFG
...
@configclass
class GaitCfg:
    gait_air_ratio_l: float = 0.37    # 左腿腾空时间比例
    gait_air_ratio_r: float = 0.37    # 右腿腾空时间比例
    gait_phase_offset_l: float = 0.37 # 左腿相位偏移 (决定左腿何时开始动作)
    gait_phase_offset_r: float = 0.87 # 右腿相位偏移 (通常与左腿错开半个周期, 0.38+0.5=0.88)
    gait_cycle: float = 0.8           # 完整步态周期的持续时间 (秒)


# =====================================================================
# 奖励函数配置 (E1_12DOF_RewardCfg)
# 定义强化学习训练中机器人的“目标”和“惩罚”。
# =====================================================================
@configclass
class E1_12DOF_RewardCfg:
    # --- 任务目标 (鼓励) ---
    track_lin_vel_xy_exp = RewTerm(func=mdp.track_lin_vel_xy_yaw_frame_exp, weight=3, params={"std": 0.5}) # 跟踪 XY 轴平移速度指令
    track_ang_vel_z_exp = RewTerm(func=mdp.track_ang_vel_z_world_exp, weight=1, params={"std": 0.5})       # 跟踪 Z 轴转向(偏航)指令

    # --- 姿态与稳定性惩罚 ---
    lin_vel_z_l2 = RewTerm(func=mdp.lin_vel_z_l2, weight=-1.0)          # 惩罚 Z 轴垂直速度 (防止机器人像兔子一样上下乱跳)
    ...
    gait_feet_spd_perio = RewTerm(func=mdp.gait_feet_spd_perio, weight=1.5, params={"delta_t": 0.02}) # 脚部速度的周期性
...
class E1_12DOF_WalkFlatEnvCfg:
    ...
    scene: BaseSceneCfg = BaseSceneCfg(
        ...
        robot=E1_12DOF_CFG,        # 导入 E1 机器人模型配置
    ...
    robot: RobotCfg = RobotCfg(
        ...
        terminate_contacts_body_names=[".*knee_link","pelvis"], # 这些部位碰地则回合失败
        feet_body_names=[".*ankle_roll_link"], # 指定脚部连杆
    )
    
    reward = E1_12DOF_RewardCfg() # 挂载上面定义的奖励
    gait = GaitCfg()              # 挂载步态参数
...
# =====================================================================
# 训练算法配置 (E1_12DOF_WalkAgentCfg)
# 定义 PPO 强化学习算法的超参数，以及 AMP (对抗运动先验) 的设置。
# =====================================================================
@configclass
class E1_12DOF_WalkAgentCfg(RslRlOnPolicyRunnerCfg):
    seed = 42
    device = "cuda:0"
    ...
    min_normalized_std = [0.05] * 12     # 动作探索噪声的最小下限