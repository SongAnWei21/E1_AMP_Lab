...
# All rights reserved.
#
# Copyright (c) 2025-2026, The E1_13DOF-Lab Project Developers.
# All rights reserved.
...
from legged_lab.envs.e1_13dof.walk_cfg import E1_13DOF_WalkFlatEnvCfg
...
class E1_13DOF_Env(VecEnv):
    ...
    def init_buffers(self):
        ...
        # 🔴 检索新增的腰部关节 ID (请确保 XML 里的名字是 waist_yaw_joint)
    ...
    def visualize_motion(self, time):
        ...
        # 🔴 ====== 解析关节位置 (13 DOF) ======
        # 假设你的 AMP txt 文件排列是：Root(3) + Euler(3) + LeftLeg(6) + RightLeg(6) + Waist(1) = 19
        dof_pos[:, self.left_leg_ids] = visual_motion_frame[6:12]      
        dof_pos[:, self.right_leg_ids] = visual_motion_frame[12:18]    
        dof_pos[:, self.waist_joint_ids] = visual_motion_frame[18:19]
        ...
        # 🔴 ====== 解析线速度与角速度 ======
        # 由于位置部分多了 1 列，后续切片需顺延 (+1)
        lin_vel = visual_motion_frame[19:22].clone()
        ang_vel = visual_motion_frame[22:25].clone()

        # 🔴 ====== 解析关节速度 (13 DOF) ======
        # 假设速度排列紧接在上面之后: LeftVel(6) + RightVel(6) + WaistVel(1)
        dof_vel[:, self.left_leg_ids] = visual_motion_frame[25:31]    
        dof_vel[:, self.right_leg_ids] = visual_motion_frame[31:37]   
        dof_vel[:, self.waist_joint_ids] = visual_motion_frame[37:38]
        ...
        return torch.cat(
            (
                ...
                self.robot.data.joint_pos[:, self.waist_joint_ids], # 🔴 添加腰部
                
                # --- 2. 关节速度 ---
                self.robot.data.joint_vel[:, self.left_leg_ids],
                self.robot.data.joint_vel[:, self.right_leg_ids],
                self.robot.data.joint_vel[:, self.waist_joint_ids], # 🔴 添加腰部
    ...
    def get_amp_obs_for_expert_trans(self):
        ...
        self.waist_dof_pos = self.robot.data.joint_pos[:, self.waist_joint_ids] # 🔴 添加腰部
        ...
        self.waist_dof_vel = self.robot.data.joint_vel[:, self.waist_joint_ids] # 🔴 添加腰部
