...
# 🔴 1. 导入 13DOF 版本的资产配置
from legged_lab.assets.e1_13dof import E1_13DOF_CFG
...
# =====================================================================
# 奖励函数配置 (E1_13DOF_RewardCfg) 🔴
# =====================================================================
@configclass
class E1_13DOF_RewardCfg:
    # --- 任务目标 (鼓励) ---
    track_lin_vel_xy_exp = RewTerm(func=mdp.track_lin_vel_xy_yaw_frame_exp, weight=3, params={"std": 0.5}) 
    track_ang_vel_z_exp = RewTerm(func=mdp.track_ang_vel_z_world_exp, weight=1, params={"std": 0.5})       
    ...
    undesired_contacts = RewTerm(
        ...
        params={
            "sensor_cfg": SceneEntityCfg(
                "contact_sensor", body_names=[".*knee_link", "pelvis"]
            ),
            "threshold": 1.0,
        },
    )
    ...
    # 🔴 新增：腰部防扭伤惩罚 (强烈建议保留，防止腰部无序旋转导致重心失控)
    ...
    # --- 周期性步态奖励 ---
    gait_feet_frc_perio = RewTerm(func=mdp.gait_feet_frc_perio, weight=1.5, params={"delta_t": 0.02}) 
    gait_feet_spd_perio = RewTerm(func=mdp.gait_feet_spd_perio, weight=1.5, params={"delta_t": 0.02}) 
    gait_feet_frc_support_perio = RewTerm(func=mdp.gait_feet_frc_support_perio, weight=1.0, params={"delta_t": 0.02})
...
# =====================================================================
# 环境配置 (E1_13DOF_WalkFlatEnvCfg) 🔴
# =====================================================================
@configclass
class E1_13DOF_WalkFlatEnvCfg:
    # 🔴 2. 更新动作文件路径到 13dof 文件夹
    amp_motion_files_display = ["legged_lab/envs/e1_13dof/datasets/motion_visualization/small_forward_boy.txt",
                                # "legged_lab/envs/e1_13dof/datasets/motion_amp_expert/walk_100fps.txt", 
                                ]
    device: str = "cuda:0"
    ...
    scene: BaseSceneCfg = BaseSceneCfg(
        ...
        num_envs=4096,             
        env_spacing=2.5,           
        robot=E1_13DOF_CFG,        # 🔴 3. 加载 13DOF 资产
    ...
    robot: RobotCfg = RobotCfg(
        ...
        action_scale=0.25,            
        terminate_contacts_body_names=[".*knee_link","pelvis"], 
        feet_body_names=[".*ankle_roll_link"], 
    )
    
    reward = E1_13DOF_RewardCfg() # 🔴 挂载新的 13DOF 奖励
    gait = GaitCfg()              
...
# =====================================================================
# 训练算法配置 (E1_13DOF_WalkAgentCfg) 🔴
# =====================================================================
@configclass
class E1_13DOF_WalkAgentCfg(RslRlOnPolicyRunnerCfg):
    seed = 42
    device = "cuda:0"
    ...
    # 🔴 4. 更新日志与实验名称
    experiment_name = "e1_13dof_walk"
    run_name = ""
    logger = "tensorboard"
    neptune_project = "e1_13dof_walk"
    wandb_project = "e1_13dof_walk"
    ...
    amp_reward_coef = 0.3 
    # 🔴 5. 更新 AMP 专家数据路径
    amp_motion_files = ["legged_lab/envs/e1_13dof/datasets/motion_amp_expert/small_forward_boy.txt",
                        # "legged_lab/envs/e1_13dof/datasets/motion_amp_expert/walk_100fps.txt",
                        ] 
    ...
    # 🔴 6. 极其关键！动作探索噪声的下限维度必须和 DOF 数量对齐 (12 -> 13)
