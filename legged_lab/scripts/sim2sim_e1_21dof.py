    ...
    class sim:
        ...
        kp = np.array([
            ...
            100, 100,  # 11,12: left/right_knee
             ...
             30,  30,  # 17,18: left/right_elbow
             20,  20   # 19,20: left/right_ankle_roll
        ], dtype=np.float32)
        
        # 🔴 严格匹配底层的阻尼 (Kd) - ONNX 顺序
        kd = np.array([
...
class MujocoRunner:
    ...
    def init_variables(self) -> None:
        ...
        # 🔴 严格匹配底层的初始姿态 (Default Pos) - ONNX 顺序
        ...
        try:
            with mujoco.viewer.launch_passive(self.model, self.data) as viewer:
                while viewer.is_running() and self.data.time < self.cfg.sim.sim_duration:
                    ...
                    # 获取并裁剪动作 (21 维)
                    raw_action = self.session.run(None, onnx_input)[0].flatten()[:21]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="/home/saw/droidup/TienKung-Lab/legged_lab/assets/e1_21dof/mjcf/E1_21dof.xml", help="Path to the MJCF/XML model file")
