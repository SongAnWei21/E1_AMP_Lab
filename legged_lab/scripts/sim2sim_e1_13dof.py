...
class SimToSimCfg:
    """针对 E1_13DOF 机器人的 Sim2Sim 配置类"""
    class sim:
        sim_duration = 10000.0  
        num_action = 13       # 🔴 升级为 13 个动作
        # 3(角速度) + 3(重力) + 3(指令) + 13(关节位置) + 13(关节速度) + 13(上一步动作) + 2(sin) + 2(cos) + 2(腾空比) = 54
        num_obs_per_step = 54 # 🔴 观测维度升级为 54
        ...
        # 🔴 严格按照 Isaac Lab 输出的顺序重排的 Kp (把腰部的插入到第3个位置)
        kp = np.array([
            100, 100,  # left_hip_pitch, right_hip_pitch
            100,       # waist_yaw_joint (腰部，给 100 的刚度)
            100, 100,  # left_hip_roll, right_hip_roll
             50,  50,  # left_hip_yaw, right_hip_yaw
            100, 100,  # left_knee, right_knee
             20,  20,  # left_ankle_pitch, right_ankle_pitch
             20,  20   # left_ankle_roll, right_ankle_roll
        ])
        
        # 🔴 严格按照 Isaac Lab 输出的顺序重排的 Kd
        kd = np.array([
            5, 5,  # left_hip_pitch, right_hip_pitch
            5,     # waist_yaw_joint (腰部阻尼)
            5, 5,  # left_hip_roll, right_hip_roll
            3, 3,  # left_hip_yaw, right_hip_yaw
            5, 5,  # left_knee, right_knee
            2, 2,  # left_ankle_pitch, right_ankle_pitch
            2, 2   # left_ankle_roll, right_ankle_roll
        ])
...
class MujocoRunner:
    def __init__(self, cfg: SimToSimCfg, policy_path, model_path):
        ...
        # 🔴 严格按照你的 ONNX TRUE JOINT ORDER 排列
        self.joint_names = [
            'left_hip_pitch_joint', 'right_hip_pitch_joint', 
            'waist_yaw_joint', # 新增的腰部偏航关节
    ...
    def init_variables(self) -> None:
        ...
        # 🔴 严格按照 ONNX 顺序排列的初始姿态 (插入腰部)
        self.default_dof_pos = np.array([
            -0.3, -0.3,  # hip_pitch (左, 右)
             0.0,        # waist_yaw (腰部默认朝正前)
    ...
    def run(self) -> None:
        print("[INFO] 🚀 Sim2Sim 13DOF Started using Passive Viewer.")
        ...
        try:
            with mujoco.viewer.launch_passive(self.model, self.data) as viewer:
                while viewer.is_running() and self.data.time < self.cfg.sim.sim_duration:
                    ...
                    # 🔴 获取并裁剪动作 (注意这里切片变成了 :13)
                    raw_action = self.session.run(None, onnx_input)[0].flatten()[:13]
...
if __name__ == "__main__":
    ...
    # 🔴 默认模型路径更新为 e1_13dof，可根据你的实际路径调整
    parser.add_argument("--model", type=str, default="/home/saw/droidup/TienKung-Lab/legged_lab/assets/e1_13dof/mjcf/E1_13dof.xml", help="Path to the MJCF/XML model file")
