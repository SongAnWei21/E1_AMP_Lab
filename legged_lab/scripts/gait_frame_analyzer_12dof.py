...
class TxtGaitAnalyzer:
    ...
    def apply_current_frame(self):
        ...
        # 12 DOF TXT 实际前 18 维是位置信息：
        # [0:3] Root Pos, [3:6] Euler XYZ, [6:18] Joint Pos (12个关节)
        
        # 1. 提取 Root 位置
        self.data.qpos[0:3] = frame_data[0:3]

        # print(f"X坐标: {frame_data[0]:.3f}, Y坐标: {frame_data[1]:.3f}")
        ...
        # 3. 提取 12 个关节的角度 [6:18] -> 放入 qpos [7:19]
        self.data.qpos[7:19] = frame_data[6:18]
...
if __name__ == "__main__":
    ...
    # 默认路径改为了 12dof，如果你实际路径不一样，启动时可通过 --xml 参数覆盖
